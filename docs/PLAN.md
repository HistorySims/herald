...
The starting demo corpus is **The New York Herald, 1840–1845**, chosen because this window captures the emergence and escalation of the **Anti-Rent Wars** — a regionally significant, nationally obscure conflict that makes a perfect "buried local story" pitch to historians, archive boards, and educators ("what's *our* town's version of this story?").

The user-facing artifact is the **original page image**, never raw OCR. OCR is the retrieval index; the page is the source of truth. Every synthesized claim links back to a specific chunk on a specific page image, and citation hallucination is a fatal flaw.

---

## 2. Corpus verification

**Confirmed via Chronicling America search results:**

- **LCCN `sn83030313` — "The New York herald" (New York, N.Y.), 1840–1920.** First issue Sep 21, 1840. OCR text, JP2 page images, and PDFs are all served by Chronicling America for issues in this range. **No fallback to Morning Herald (sn83030312) is needed.**
- Predecessor titles for context (not in scope for Phase 1):
  - `sn83030311` — "The Herald" (New York, N.Y.), 1835–1837
  - `sn83030312` — "Morning herald" (New York, N.Y.), 1837–1840

The Anti-Rent War (a.k.a. Helderberg War) ran ~1839–1845; our window (Sep 1840 onward) catches the post-Stephen-Van-Rensselaer-III escalation, the rise of the Calico Indians (1844), Sheriff Osman Steele's killing at Moses Earle's farm (Aug 1845), and Governor Silas Wright's 1845 crackdown — all within the corpus.
...
```
        ...
        └─────────────────────────────────────────────────────────────────────────┘
                            ...
                            │  - structured chunks w/ stable IDs        │
...
### 4.1 Ingestion (Phase 1, run once over 1840–1845)

1. **Enumerate.** Use the Chronicling America batch/JSON API to list every issue of `sn83030313` with `date_issued` in `[1840-09-21, 1845-12-31]`. For each issue, list its pages.
...
8. **Checkpoint.** Resume safely on crash: ingestion is idempotent keyed on `(lccn, date, sequence)` for pages and `(page_id, chunk_index, ocr_version)` for chunks.

### 4.2 Retrieval (Phase 1 CLI; Phase 2 web)

1. Embed the user query with Voyage (same model as corpus).
2. **Semantic top-k** (`k_sem = 50`) via HNSW cosine.
...
6. **MMR diversity** (λ=0.5) over the top 20 → final 8–12 chunks for synthesis. MMR is applied conditionally: synthesis-style queries ("how does the Herald characterize X across this period?") benefit; narrow factual queries skip it.
7. Pass chunks to the synthesizer with stable IDs and full citation metadata.

### 4.3 Synthesis & display

The LLM gets a numbered chunk dossier and a strict "cite-or-refuse" system prompt (§9). It returns an answer with inline citation markers (`[3]`, `[7]`) that map back to chunks the retrieval layer chose. The CLI (Phase 1) prints answer + numbered source list with date, page, sequence, and the LOC page image URL. The Phase 2 UI clicks a citation → opens that page image in the right-hand viewer at the chunk's approximate location.
...
create table papers (
  ...
  lccn          text unique not null,           -- e.g. 'sn83030313'
  title         text not null,                  -- 'The New York Herald'
...
create table reocr_jobs (
  ...
  created_at      timestamptz not null default now(),
  unique (page_id, prev_ocr_version) where status in ('queued','running')
...
- **`unique (page_id, prev_ocr_version) where status in ('queued','running')`** on `reocr_jobs` — prevents two users from simultaneously paying to clean the same page version.

---

## 6. Chunking strategy

**Decision: 400 words / 50-word overlap, fixed window, on per-page text.** Approve as proposed by user; this lands in the modern RAG sweet spot (~500–700 tokens, 10–15% overlap).
...
- **One page → ~10–15 chunks** at ~5,000 words/page (1840s broadsheet density). Across the corpus this comes out to roughly **100k–150k chunks**.
...
- **MMR step** — applied for breadth-y queries (heuristic: query contains "how", "what kinds of", "across", "compare", or no proper noun). λ=0.5. Skipped for narrow factual queries to avoid bleeding away the best hit.
- **Date / paper filters** — accept optional `date_from`, `date_to`, `paper_id`. Filters apply *before* HNSW search via `WHERE` clause on the partial-index'd query.
...
> You are a research assistant grounded in a specific newspaper corpus. You will be given a numbered list of source passages (chunks) from *The New York Herald* between 1840 and 1845. Each chunk has an ID, a date, and a page reference. Answer the user's question using **only** these passages.
>
...
SOURCES:
[1] 1844-08-12, p.2, seq 2, chunk a3f… —
    <chunk text>

[2] 1845-08-09, p.1, seq 1, chunk 9c4… —
    <chunk text>
...
```
...
### Per-query (retrieval + synthesis), Phase 1 and beyond
...
### One-time corpus ingest cost (NY Herald, 1840–1845)

Assumptions:

- ~313 issues/year × 6 years ≈ **~1,880 issues**
- ~4–5 pages/issue avg (the Herald grew over our window) → **~9,000 pages**
- ~5,000 words/page (1840s broadsheet density) → **~45M words ≈ ~60M tokens**

| Component | Cost |
|---|---|
| Voyage `voyage-3.5` embeddings (60M tokens) | **~$3.60** |
| Egress / LOC fetch | $0 (LOC is free, public-domain) |
| Supabase storage (~3 GB chunks + vectors + tsvectors) | low-double-digits/month at most |
| **One-time ingest** | **< $10** |

The corpus is cheap. The product economics live in **per-query synthesis** and **per-page Cleanup**, which are both modest.
...
1. **"Find references to the Helderberg disturbances and the Calico Indians in the New York Herald, 1840–1845."** — Tests basic semantic recall on the corpus's signature event.
2. **"How does the Herald characterize tenants versus landlords across this period?"** — Tests breadth-y synthesis; MMR should kick in.
3. **"Trace the Herald's coverage of Stephen Van Rensselaer III's death and its aftermath."** — Tests temporal reasoning across multiple issues.
4. **"What language does the paper use for tenant violence versus landlord property claims? Quote specific phrasings."** — Tests close-reading via exact-quote retrieval; FTS leg must carry weight.
5. **"Identify the named anti-rent leaders who appear repeatedly in the Herald, and describe how each is characterized."** — Tests entity-level recall (Smith Boughton / "Big Thunder", Moses Earle, Osman Steele, Silas Wright, John Young, Stephen Van Rensselaer IV).
6. **"What does the Herald say about the killing of Sheriff Osman Steele at Andes in August 1845?"** — Tests precise event-level retrieval; date-filtered slice should work too.
7. **"How does the Herald frame the 1845 anti-disguise law and Governor Wright's crackdown?"** — Tests political/editorial-stance extraction.
8. **"Do the Anti-Rent Wars share newspaper real estate with national stories like the 1844 election or the Mexico crisis? What gets bumped, what gets prominence?"** — Tests cross-topic synthesis and page-prominence reasoning.
9. **"Refuse gracefully: what does the Herald say about the discovery of helium?"** — Negative-case test. Helium is post-corpus; the model must refuse, not confabulate. (Citation-validation safeguard from §9 should also flush hallucinated marker IDs.)
10. **"Find any reference to a specific upstate town — e.g. Berne, Rensselaerville, or Andes — and summarize what the Herald says happened there."** — Tests the demo's emotional pitch: "what's our town's version of this story?"
...
- Ingestion CLI: `herald ingest --lccn sn83030313 --from 1840-09-21 --to 1845-12-31`. Resumable, idempotent, batched, with progress reporting.
- Chunker (400/50, sentence-snapping).
- Voyage embedding client with batch + retry.
- Retrieval module: hybrid HNSW + FTS + RRF + rerank + optional MMR.
...
- Vision-OCR worker (Claude Sonnet 4.6 vision is the leading candidate; final pick is a Phase-3-day-1 bake-off vs. GPT-class alternatives on a 50-page sample).
...
- **LOC OCR quality variance.** Some pages are unreadable. Phase 1 ingest must not crash on near-empty OCR; chunks below a minimum-content threshold get dropped, not embedded. Acceptable retrieval coverage is an empirical question we won't fully answer until eval.
...
- **HNSW recall at 100k+ chunks.** HNSW defaults are usually fine, but if recall@10 is bad on validation questions we may need to tune `m`/`ef_construction`/`ef_search`. Worth measuring early.
...
- **Multi-paper future.** The schema supports multiple papers from day one (`papers` table, `paper_id` FK on issues). Nothing in Phase 1 should hardcode `sn83030313`.

---

## 15. Verification (how we'll know Phase 1 works)

1. `herald ingest --lccn sn83030313 --from 1840-09-21 --to 1845-12-31` completes end-to-end with no manual intervention; row counts in `papers`/`issues`/`pages`/`chunks` match LOC's totals within ±0.5%.
2. The Supabase project has all 7 tables, all indexes (HNSW + GIN + btree), and explain-analyze on a sample query shows index use.
3. `herald ask "<q>"` returns answer + citations + image URLs for each of the 10 validation questions in §12.
...
When all six are green, Phase 1 is done and we open the Phase 2 plan PR.
